...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Watermark
    const wm = "PAYMENT DUE";
    page.drawText(wm, { x: width / 2 - boldFont.widthOfTextAtSize(wm, 42) / 2, y: pageH / 2, size: 42, font: boldFont, color: rgb(0.97, 0.85, 0.85), rotate: degrees(45) });

    let y = pageH - 60;
    page.drawText("PAYMENT REMINDER", { x: 50, y: pageH - 40, size: 11, font: boldFont, color: rgb(0.92, 0.15, 0.15) });
    ...
    if (quote.valid_until) page.drawText(`Due: ${new Date(quote.valid_until as string).toLocaleDateString()}`, { x: width - 200, y: csY - 33, size: 10, font: boldFont, color: rgb(0.92, 0.15, 0.15) });
    ...
    cp.drawText("Total Due:", { x: lx, y, size: 14, font: boldFont, color: rgb(0.92, 0.15, 0.15) });
    cp.drawText(`${C}${Number(quote.total).toFixed(2)}`, { x: vx, y, size: 14, font: boldFont, color: rgb(0.92, 0.15, 0.15) });

    const pdfBytes = await pdfDoc.save();
    return new NextResponse(new Uint8Array(pdfBytes), {
      headers: { "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="Reminder_${quote.quote_number}.pdf"` },
    });
  } catch (error) {
