  ...
  useEffect(() => {
    ...
    async function loadQuotes() {
      ...
      try {
        ...
        const response = await fetch(`/api/quotes/list?${query}`, {
    ...
    <section className="workspace-page">
      ...
      {isEmpty ? (
        <div className="state-panel">
          ...
          {statusFilter ? (
            <button className="button button--secondary" type="button" onClick={() => setStatusFilter("")}>
              Clear filter
            </button>
          ) : (
            <Link className="button button--primary" href="/quotes/new">
      ...
      {state.status === "success" && quotes.length > 0 ? (
        <div className="table-card">
          ...
          {quotes.map((quote) => (
            <Link
              className="quote-table quote-table--row"
              key={quote.id}
            ...
            >
              ...
              <button
                ...
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  setDeleteTarget({ id: quote.id, number: quote.quoteNumber });
                }}
              >
                Delete
              </button>
      ...
      <ConfirmDialog
        ...
        title="Delete quote"
        message={`Are you sure you want to delete ${deleteTarget?.number ?? "this quote"}? This action cannot be undone.`}
        confirmLabel="Delete"
        variant="danger"
